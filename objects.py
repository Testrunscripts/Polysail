...
class Island(StationaryObject):
	def __init__(self, name=None, x=None, y=None, size=None):
		...
		self.size = size or random.randint(200, 800)
	...
	def draw(self, screen, cam_x, cam_y):
		...
		if (offset_x + self.size < 0 or offset_x - self.size > sett.WIDTH or offset_y + self.size < 0 or offset_y - self.size > sett.HEIGHT):
			return  # off-screen
		#screen.blit(self.surface, (offset_x - self.size, offset_y - self.size))
...
class Wake:
	...
	def update(self, dt):
		...
		"""if self.color[0] > 100:
			self.color[0] = max(self.color[0] - 1, 100)
		else:
			self.color[1] = max(self.color[1] - 1, 200)
		#self.color[2] = max(self.color[2] - 5, 200)"""
